# PHI TONG | Senior LXD Architect & Full-Stack Learning Engineer
**Ho Chi Minh City, Vietnam** | **phitong.official@gmail.com** | **+84 (799) 445 377**
**LinkedIn:** [hoang-phi-tong](https://www.linkedin.com/in/hoang-phi-tong) | **Portfolio:** [Senior Architect Portfolio](https://hoangphi-id.github.io)

## EXECUTIVE SUMMARY
- **Senior Instructional Designer** with 10+ years of experience designing scalable learning systems and high-impact digital content for corporate EdTech and language training institutions.
- **Digitized** over **500 pedagogical assets** into a modular LMS environment, implementing automated gating and activity completion tracking to improve learner autonomy.
- **Architected** an end-to-end **36-session curriculum framework** for English proficiency, optimizing learning paths for personalized education.
- **Engineered** custom **JavaScript-to-API Bridges** for interactive simulations, enabling precision tracking of learner behavior via **Asynchronous POST methods** and **Serverless Data Warehouses**.

## PROFESSIONAL EXPERIENCE

### Academic Quality & Curriculum Architect | Sep 2023 – Jun 2025
**Universal Study Center (USC)** — Ho Chi Minh City, Vietnam
*USC is an international education consultancy specializing in standardized test prep (IELTS/SAT/GRE) for high-performance learners entering global top-tier universities.*
- **Spearheaded** the digital transformation of high-stakes IELTS curricula, managing a cross-functional team of 4-6 educators to ensure 100% brand-architectural alignment.
- **Engineered** standardized assessment frameworks and interactive modules, achieving a **15% increase in high-band (7.5+) candidate attainment rates**.
- **Automated** placement and student tracking workflows, reducing manual diagnostic and processing lead times by **40%**.
- **Orchestrated** pedagogical strategy for merit-based candidates, facilitating over **$10,000+ in university scholarship funding**.
- **Synthesized** learner data from 100+ active candidates to inform curriculum architecture pivots and adaptive instructional execution.

### Senior Instructional Designer & Content Architect | Nov 2021 – May 2023
**EngTalk Language Center** — Ho Chi Minh City, Vietnam
*EngTalk is a digital-first EdTech platform focusing on professional communication and language training for corporate learners.*
- **Architected** a scalable **36-session hybrid syllabus framework** from conception to market-ready launch for adult learners.
- **Led** the production of 200+ micro-learning instructional assets, utilizing Articulate Rise 360 and advanced Storyline animations.
- **Engineered** interactive Storyline blocks with custom **ES6 Logic Gates** to track high-stakes decision-making and pipe data directly to external analytics dashboards.
- **Developed** a centralized content hub on Notion, streamlining asset management for a distributed team of 20+ instructors.
- **Collaborated** with business units to align curriculum goals with a **20% improvement in customer retention targets**.

### Senior Instructional Designer & Product Lead | Mar 2021 – Oct 2021
**VietAccepted** — Ho Chi Minh City, Vietnam
*VietAccepted is a premier training hub for GMAT/GRE and English certifications for high-potential professionals.*
- **Managed** the end-to-end production of brand-consistent instructional assets and visual learning modules for 1,000+ adult learners.
- **Standardized** ID workflows by implementing reusable storyboard templates, reducing content development cycles by **20%**.
- **Conducted** data-driven audits of instructional modules to identify and resolve learner friction points in complex GMAT/GRE concepts.
- **Consolidated** a centralized repository of 1,000+ reusable instructional assets for rapid product deployment and brand consistency.

### LMS Architect & Experience Designer | Jul 2019 – Oct 2020
**Vietop English Center** — Ho Chi Minh City, Vietnam
*Vietop is an English training center specializing in standardized test preparation with a focus on personalized learning paths.*
- **Digitized** 500+ static paper-based assets into a modular Moodle environment with automated rubrics and conditional access gating.
- **Optimized** the course taxonomy for IELTS preparation, improving student self-service efficiency by **25%**.
- **Resolved** LMS accessibility issues by developing mobile-responsive CSS and streamlined SCORM packages for Moodle.
- **Engineered** automated grading systems and **Pedagogical Logic Gates**, significantly reducing manual instructor feedback cycles while maintaining data fidelity.

### Senior ID & Curriculum Lead | Sep 2017 – Jun 2019
**ZIM Academy** — Ho Chi Minh City, Vietnam
*ZIM is an academic institution providing intensive English preparation and personalized educational consulting.*
- **Led** trainer onboarding and instructional audits for **6+ new teachers** to ensure methodological consistency across 4+ branches.
- **Implemented** systematic classroom observation and feedback cycles, improving standardized test performance across key student cohorts.
- **Authored** 10+ comprehensive student workbooks and teacher guides, standardizing the IELTS Writing delivery system across the academy.

## PORTFOLIO PROJECTS
- **The Deadline Crisis Simulator (Storyline 360):** Advanced branching simulation with custom **JavaScript data bridge** to Google Sheets API for real-time tracking.
- **Universal LMS Scalability (Moodle LMS):** End-to-end automated LMS architecture with complex activity gating, mastery-based feedback, and adaptive paths.
- **Modern Employee Onboarding (Rise 360):** Aesthetic technical onboarding case study highlighting agile deployment and mobile-first UX principles.

## EDUCATION & CERTIFICATIONS
**Ho Chi Minh City Open University**, *B.A. English* (2019-23) | **Hoa Sen University**, *B.S. IT* (2010-16)
**Certifications:** IELTS Teacher Training (IDP & BC) | TESOL | Cambridge Teaching English Online

## SKILLS & TECH
SKILLS: Instructional Design, Curriculum Development, LMS Architecture, SCORM/xAPI, Technical Writing, Storyboarding.
TECH: JavaScript (ES6), ES6 Data Bridges, Asynchronous Pipelines, Moodle LMS (Admin), Articulate Storyline/Rise (Advanced JS), Canvas, GenAI Automation (Make.com/Zapier).
